...
} from 'src/repositories/mongoose-content.repository';
...
export class UserService {
  constructor(
    ...
    private readonly movieRepository: MongooseMovieContentRepository,
    @Inject('TVShowContentRepository')
    private readonly tvshowRepository: MongooseTVShowContentRepository,
  ...
  async likeContent(
    ...
    const userExists: string = await this.redisService.get(id);
    let user: User;
    if (!userExists) {
    ...
    const existingContent = user[fieldToUpdate]?.find(
  ...
  async removelikedContent(
    ...
    return this.userRepository.updateLiked(
      id,
      contentId,
      contentType,
      'remove',
  ...
  async getLikedContent(id: string, page = 1, pageSize?: number): Promise<any> {
    const content = [];
    const userExists: string = await this.redisService.get(id);
    let user: User;
    if (!userExists) {
    ...
    if (user?.likedMovies?.length) {
      const movies = await this.movieRepository.listAll(contentIds);
      content.push(...movies);
    }

    if (user?.likedTvShows?.length) {
      const tvshows = await this.tvshowRepository.listAll(contentIds);
  ...
  async validateContentIdExists(
    contentType: string,
    contentId: string,
  ): Promise<boolean> {
    const isValid = false;
    if (contentType == 'movie') {
      const content = await this.movieRepository.findOne(contentId);
      if (content) return true;
    } else if (contentType == 'tvshow') {
      const content = await this.tvshowRepository.findOne(contentId);
      if (content) return true;
    }
    return isValid;
  }
