...
export class ContentService {
    constructor(
        @Inject('MovieContentRepository') private readonly movieContentRepository: ContentRepository,
        @Inject('TVShowContentRepository') private readonly tvshowContentRepository: ContentRepository,
    ) { }
    ...
    async create(contentType: 'movie' | 'tvshow', content: Movie | TVShow): Promise<Movie | TVShow> {
    ...
    async findById(contentType: 'movie' | 'tvshow', id: string): Promise<Movie | TVShow> {
        const repository = this.getRepository(contentType);
        return repository.findOne(id);
    }

    update(contentType: 'movie' | 'tvshow', id: string, content: Partial<Movie | TVShow>): Promise<Movie | TVShow> {
        const repository = this.getRepository(contentType);
